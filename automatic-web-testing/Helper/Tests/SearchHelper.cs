...
namespace automatic_web_testing.Helper.Tests
{
    public class SearchHelper
    {
        ...
        public static IList<IWebElement> WaitForElementsByClassName(string className, IWebElement driver, int timeout, int pollinginterval)
        {
            if (driver == null) return null;
            IWebElement tmp = null;
            while (tmp == null)
            {
                tmp = driver.FindElement(By.ClassName(className));
            }
        ...
        /// <param name="pollinginterval"></param>
        /// <returns></returns>
        public static IWebElement WaitForElementByTagName(string tagName, ChromeDriver driver, int timeout, int pollinginterval)
        {
            if (driver == null) return null;
            var wait = new WebDriverWait(driver, TimeSpan.FromSeconds(timeout));
            wait.PollingInterval = TimeSpan.FromMilliseconds(pollinginterval);
        ...
        /// <param name="pollinginterval"></param>
        /// <returns></returns>
        public static IList<IWebElement> WaitForElementsByTagName(string tagName, ChromeDriver driver, int timeout, int pollinginterval)
        {
            if (driver == null) return null;
            var wait = new WebDriverWait(driver, TimeSpan.FromSeconds(timeout));
            wait.PollingInterval = TimeSpan.FromMilliseconds(pollinginterval);
